...
var DEFAULT_API_URL2 = "https://api.alpha.candle.tv";
...
function quoteIdDecimals(quoteAsset, chain2, extra) {
  const quote = quoteAsset ?? defaultQuoteId(chain2);
...
async function executeTrade(args, cfg, doFetch) {
  ...
  try {
    if (args.side === "buy") {
      ...
      const address = chain2 === "hood" ? walletsBody.wallets?.evm?.address : walletsBody.wallets?.solana?.address;
      if (!address) {
        return errText(`percent sells need an embedded ${chain2 === "hood" ? "EVM" : "Solana"} wallet, and this account has none`, { clientTradeId });
      }
...
function buildRequest(name, args, cfg) {
  ...
  switch (name) {
    ...
    case "candle_get_market": {
      const { chain: chain2, mint } = args;
      return {
        url: `${base2}/api/v1/markets/${encodeURIComponent(chain2)}/${encodeURIComponent(mint)}`,
    ...
    case "candle_token_forensics": {
      const { chain: chain2, mint } = args;
      return {
        url: `${base2}/api/v1/markets/${encodeURIComponent(chain2)}/${encodeURIComponent(mint)}/forensics`,
        init: { method: "GET", headers: jsonHeaders() }
      };
    }
...
var init_tools = __esm(() => {
  ...
  launchTokenShape = {
    ...
    imageUrl: exports_external.string().describe("https URL to the token image. Must be roughly SQUARE (aspect ratio at most 1.5:1): it " + "renders as a small circle/square avatar everywhere. Share cards, OG images and banners " + "are rejected with IMAGE_WRONG_SHAPE -- pass those as bannerUrl instead"),
    bannerUrl: exports_external.string().optional().describe("Optional https URL to WIDE artwork for the token page's banner strip (wider than 1.5:1, " + "e.g. 1200x630). This is where a share card or OG image belongs. A square image here is " + "rejected with BANNER_WRONG_SHAPE. Omit it and the strip falls back to imageUrl"),
    chain: exports_external.string().optional().describe('"solana" or "hood"; defaults to solana'),
    quoteAsset: exports_external.string().optional().describe("Quote asset symbol; defaults per chain"),
    mode: exports_external.string().optional().describe('"open" or "exclusive"; defaults to open. "test-open" / "test-exclusive" launch the ' + "low-threshold test curves (~1/80 economics) where the API has ENABLE_TEST_CURVES on"),
    stakerAllocationBps: exports_external.number().optional().describe("Staker allocation in basis points"),
    dexVersion: exports_external.string().nullable().optional().describe('"v3" or "v4"; required for hood launches'),
    buyAmount: exports_external.union([exports_external.string(), exports_external.number()]).optional().describe("Initial buy, in quote base units (string or number)"),
    description: exports_external.string().optional(),
    socials: exports_external.record(exports_external.string()).optional().describe("Social links keyed by platform"),
    visibility: exports_external.string().optional().describe('"production", "test", "local", or "hidden"'),
    dryRun: exports_external.boolean().optional().describe("Validate without executing the launch")
  };
  getMarketShape = {
  ...
  reportActivityShape = {
    chain: exports_external.string().describe('"solana" or "hood"'),
    signature: exports_external.string().describe("Transaction signature/hash to verify and record")
  };
  ...
  tradeShape = {
    ...
    quoteAsset: exports_external.string().optional().describe('Quote asset ("sol", "usdc", "cndl") for an arbitrary Solana mint Candle never launched ' + "(Pro/Max only); defaults to sol. Ignored for a Candle-launched token, whose quote comes " + "from the token itself, so it never changes how a buy amount is interpreted there."),
  ...
  transferShape = {
    ...
    asset: exports_external.enum(["SOL", "USDC", "CNDL", "ETH", "USDG"]).optional().describe("A base asset key. Pass exactly one of asset or mint."),
    mint: exports_external.string().optional().describe("An arbitrary token: SPL mint (Solana) or ERC-20 contract (Hood). Own-wallet destinations only; withdrawals to approved addresses are base-assets-only."),
    amountRaw: exports_external.string().describe('RAW base units as a decimal string (lamports, wei, token raw units), or "max" to sweep the spendable balance. NOT a human decimal: 1 SOL is "1000000000".'),
    to: exports_external.string().describe("Destination address. Must be one of the account's own wallets, or an address the OWNER pre-approved as a withdrawal address in the Candle console -- anything else is refused before signing."),
    clientTransferId: exports_external.string().optional().describe("Caller-chosen idempotency key for safe retries")
  };
  sweepShape = {
    chain: exports_external.enum(["solana", "hood"]).describe("Which chain to sweep"),
    to: exports_external.string().describe("Destination address, same rules as candle_transfer's `to`"),
