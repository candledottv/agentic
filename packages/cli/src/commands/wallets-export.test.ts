...
// ── Ember Phase 1 (BE-94, T26): the TEE wallet store has no plaintext export path ─────────────────────

describe("wallets export refuses the Ember TEE wallet store", () => {
  test("--yes against tee-wallets.enc prints no key and names the sweep", async () => {
      ...
      [
        {
          index: 0,
          chain: "solana",
          address: "TeeAddr11111111111111111111111111111111111",
          label: "tee-0",
    ...
    const code = await run(["wallets", "export", "--index", "0", "--yes", "--keystore", "/x/tee-wallets.enc"], deps)
    expect(code).toBe(1)
    expect(stdout.text).not.toContain("5SecretThatMustNeverPrint")
    expect(stderr.text).toContain("tee sweep")
  })
})
