...
class CandleClient {
  ...
  async trade(req) {
    ...
    if (!this.evmRpcUrl) {
      throw new Error('trade({ from: <linked> }) for chain "hood" requires evmRpcUrl: pass one in CandleClientOptions ' + "(new CandleClient({ evmRpcUrl }))");
    }
  ...
  async selfLaunch(req) {
    ...
    if (!this.evmRpcUrl) {
      throw new Error('selfLaunch() for chain "hood" requires evmRpcUrl: pass one in CandleClientOptions ' + "(new CandleClient({ evmRpcUrl }))");
    }
    const rpc = this.evmRpc();
